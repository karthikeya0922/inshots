...
${audioHtml.join("\n")}
    ...
    <script>
      // Generated by Frameo for "${escapeHtml(dna.name)}". Palette, type, radius and motion derive from the analyzed product.
...
`;
  ...
  await writeText(path.join(compositionDir, "package.json"), JSON.stringify({ name: `${slug(dna.name)}-frameo`, private: true, type: "module", scripts: { dev: `npx --yes hyperframes@${input.hyperframesVersion} preview`, check: `npx --yes hyperframes@${input.hyperframesVersion} check`, render: `npx --yes hyperframes@${input.hyperframesVersion} render` } }, null, 2) + "\n");
  await writeText(path.join(compositionDir, "meta.json"), JSON.stringify({ id: `${slug(dna.name)}-launch`, name: `${dna.name} launch video`, createdAt: new Date().toISOString(), generator: "frameo" }, null, 2) + "\n");
